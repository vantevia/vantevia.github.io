...
const useSongDisplay = (props: any) => {
  const { song: s, showRevisionHistory: rev, sortMode, revisionMainRankMode } = props;
  ...
  const styles = useMemo(() => {
    ...
    // Toggles for border and background are removed, defaulting to ON when not in revision history.
    return {
      specific: spec,
      activeBorder: (!rev && valid) ? spec : gray,
      containerBg: (!rev && valid) ? spec.backgroundStyle : gray.backgroundStyle
    };
  }, [s, rev, isRank]);
...
const ClassicItem = (p: any) => {
  const { song, isCompact, showDetails, hideTierText } = p, display = useSongDisplay({ ...p, song });
  return (
      ...
      <div className="flex-1 min-w-0">
        <SimpleItem {...p} styles={{ ...display.activeBorder, backgroundStyle: display.containerBg, textStyle: display.specific.textStyle }} customRankDisplay={p.customRankDisplay || (display.mainRank !== null ? `#${display.mainRank}` : '')}
          rightContent={(<>
              ...
              {!hideTierText && !song.isLegacy && !song.isUnranked && song.tier && <div className={`${isCompact ? 'w-16' : 'w-24'} text-center shrink-0 rounded-none`}><p className={`${isCompact ? 'text-xl' : 'text-4xl'} font-black drop-shadow-md leading-none`} style={display.specific.textStyle}>{song.tier}</p></div>}
          </>)} />
      </div>
...
const GridCompactItem = (p: any) => {
  const { song, onClick, hideTierText, showArtist = true } = p, display = useSongDisplay({ ...p, song });
  return (
...
const GridWideItem = (p: any) => {
  const { song, onClick, hideTierText, showArtist = true, showVisualMetadata = true, showDetails, isEditing, onUpdateRank, onUpdateProp } = p;
  ...
  return (
    <div onClick={isEditing ? undefined : onClick} className={`group relative w-full ${showDetails ? 'min-h-[200px]' : (isCondensed ? 'h-24' : 'h-36')} rounded-none ${isEditing ? 'cursor-default' : 'cursor-pointer'} shadow-lg transition-transform hover:scale-[1.01] p-[3px]`} style={display.activeBorder.borderStyle}>
